    ...
    def restaurant_info(self, cards):
        try:
            ...
            return {
                ...
                "name": info.get("name"),
                "city": info.get("city"),
                "latLong": map(float, str(info.get("latLong")).strip().split(",")),
                'address': info.get('labels')[1]['message'],
    ...
    try:
        ...
        if restaurants:
            for rest in restaurants:
                restaurant = Restaurant(rest['id'])
                menu_data = restaurant.get()
